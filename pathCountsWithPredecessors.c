        ...
        for (unsigned int j = 0; j < nbNodes; j++) {
            for (unsigned int k = compact->offsets[j]; k < compact->offsets[j + 1]; k++) {
                if (compact->predecessors[k] == i) {
                    pathCounts->data[i * sumDegrees + k] = compact->weights[k];
                } else {
                    pathCounts->data[i * sumDegrees + k] = 0;
                }
            }
        }
    }
...
pathCountsWithPredMatrix *buildNextPathCounts(pathCountsWithPredMatrix *pathCounts, compactAdjacencyMatrix *compact) {
    ...
    for (unsigned int i = 0; i < nbNodes; i++) {
        for (unsigned int j = 0; j < nbNodes; j++) {
            float sum = 0;
            for (unsigned int k = compact->offsets[j]; k < compact->offsets[j + 1]; k++) {
                sum += pathCounts->data[compact->predecessors[k] * sumDegrees + k];
            }
            nextPathCounts->data[i * sumDegrees + compact->offsets[j]] = sum;
        }
    }
